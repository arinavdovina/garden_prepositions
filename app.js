...
function render(){
 ...
 layout.forEach((id,i)=>{
  ...
  b.setAttribute('aria-label',`${i<4?'Back':'Front'} row, patch ${i%4+1}${id?': '+NAMES[id]+', stage '+(growth[id]+1):', empty'}`);
  if(id){b.innerHTML=`<img src="${asset(id,growth[id])}" style="height:${[45,65,82,100][growth[id]]}%" alt=""><span class="plant-name">${NAMES[id]}</span><span class="stage-dots" aria-hidden="true">${'●'.repeat(growth[id]+1)}${'○'.repeat(3-growth[id])}</span>`;b.draggable=mode!=='grow'||tool==='move';}
 ...
 PLANTS.forEach(id=>{const b=document.createElement('button');b.className='seed'+(selected===id?' selected':'')+(layout.includes(id)?' placed':'');b.disabled=layout.includes(id);b.draggable=!b.disabled;b.dataset.plant=id;b.innerHTML=`<img src="${asset(id,0)}" alt=""><span>${NAMES[id]}</span>`;b.onclick=()=>{selected=id;render();say(`Put your ${NAMES[id].toLowerCase()} on a patch of soil.`);};b.ondragstart=e=>{dragging=id;e.dataTransfer.setData('text/plain',id);};b.ondragend=()=>dragging=null;$('plant-list').append(b);});
 ...
 $('harvest-count').textContent=`${grown.length} / 8`;$('harvest-items').innerHTML=grown.map(p=>`<img src="assets/fruit-${p}.webp" alt="${NAMES[p]}" title="${NAMES[p]}">`).join('');
...
function choosePlot(i){const id=layout[i];if(mode==='grow'&&tool==='water'){if(id)ask(id);return;}if(selected){moveTo(i);return;}if(id){selected=id;render();say(`Move your ${NAMES[id].toLowerCase()}. Tap another patch.`);}else say('Choose a plant below first.');}
...
$('check-final').onclick=()=>{const result=checkGarden(layout);document.querySelectorAll('#clues li').forEach((li,i)=>{li.className=result[i]?'correct':'incorrect';li.setAttribute('aria-label',(result[i]?'Correct: ':'Try again: ')+CLUES[i].text);});if(result.every(Boolean)){$('win').showModal();say('You solved every clue!');}else say(`${result.filter(Boolean).length} / 8 clues solved. Check the marked clues and try again.`);};
